...
# parser.add_argument('-log','--log_dir', required=True, default='log_itrPCRNet_ours', help='Log dir [default: log]')
...
parser.add_argument('-log','--log_dir',  default='log_itrPCRNet_ours', help='Log dir [default: log]')
...
parser.add_argument('--model_with_noise', type=int, default=0, help='Oure model with noise version')
...
ADD_NOISE_MODEL = bool(FLAGS.model_with_noise)
print(FLAGS.Noise)
...
def train():
	...
	with graph.as_default():
		...
		with tf.device('/gpu:'+str(GPU_INDEX)):
			...
			transformed_predicted_point_cloud = helper.transformation_quat_tensor(source_pointclouds_pl,
																				  predicted_norm_quat,
																				  predicted_position)

			is_training_pl_1 = tf.placeholder(tf.bool, shape=())  # flag for batch norm

			# load our model as loss function:
			saver31 = tf.train.import_meta_graph(os.path.join(LOG_DIR, 'model.ckpt.meta'),
												 import_scope='g1',
												 input_map={'input1': transformed_predicted_point_cloud,
			...
			loss_c = chmafer_dist(transformed_predicted_point_cloud, template_pointclouds_pl)
