...
def apply_transformation_old(datas,poses):			# Transformation function for (2 & 4c, loss 8b)
	#Old version of 'apply_transformation' by Sarode
	# Arguments:
	...
	transformed_data = np.zeros(datas.shape, dtype=np.float32)
	for i in range(datas.shape[0]):
		transformed_data[i,:,:] = rotate_point_cloud_by_angle_z(datas[i,:,:],poses[i,5])
...
def apply_transformation(datas,poses):
	...
	faster implementation of 'apply_transformation' by Sarode
	:param datas:
	:param poses:
	:return:
	"""
	rotated_data = np.zeros(datas.shape, dtype=np.float32)
	for i in range(datas.shape[0]):
		rz = poses[i,5]
		ry = poses[i,4]
		rx = poses[i,3]
		...
		Rz = np.array([[np.cos(rz), -np.sin(rz), 0],
					...
					[0, 0, 1]])
		#R = np.dot(Rz, np.dot(Ry, Rx)) #Suffer from quantiztion
		t_xyz = (poses[i,0],poses[i,1],poses[i,2])
		rotated_data[i, ...] = np.dot(Rz, datas[i,...].reshape((-1, 3)).T).T 		# Pre-Multiplication (changes done)
		rotated_data[i, ...] = np.dot(Ry, rotated_data[i,...].reshape((-1, 3)).T).T 		# Pre-Multiplication (changes done)
		rotated_data[i, ...] = np.dot(Rx, rotated_data[i,...].reshape((-1, 3)).T).T 		# Pre-Multiplication (changes done)
		#rotated_data[i, ...] = np.dot(R, datas[i,...].reshape((-1, 3)).T).T + t_xyz	# Suffer from quantiztion
		rotated_data[i,...] = rotated_data[i, ...] + t_xyz
	return rotated_data
...
def transformation_quat2mat(poses,TRANSFORMATIONS,templates_data):		# (for 4b)
	...
	for i in range(poses.shape[0]):
		...
		TRANSFORMATIONS[i,:,:] = np.dot(transformation_matrix,TRANSFORMATIONS[i,:,:])		# 4b (Multiply tranfromation matrix to Initial Transfromation Matrix) (Homogenous Coordinates)
...
def normalize_vector(x):
    x_size = tf.sqrt(tf.reduce_sum(tf.square(x),-1,keepdims=True))
    return x/x_size
def cross_product(x,y):
    return tf.linalg.cross(x,y)
def transformation_6D_tensor(data,ortho6d):
	# Arguments:
		# data:					Tensor of Point Cloud. (batch_size x num_point x 3)
		# ortho6d:					6D representation tensor to generate rotation matrix.	(batch_size x 6)
		# translation:			Translation tensor to translate the point cloud. (batch_size x 3)
	# Outputs:
		# transformed_data: 	Tensor of Rotated and Translated Point Cloud Data. (batch_size x num_points x 3)
	x_raw = ortho6d[:, 0:3]  # batch*3
	y_raw = ortho6d[:, 3:6]  # batch*3
	translation = ortho6d[:, 6:9]
	x = normalize_vector(x_raw)  # batch*3
	z = cross_product(x, y_raw)  # batch*3
	z = normalize_vector(z)  # batch*3
	y = cross_product(z, x)  # batch*3

	Rmatrix = tf.stack([x,y,z],-1)

	transformed_data = []		# Tensor to store transformed data.
	for i in range(ortho6d.shape[0]):
		temp_rotated_data = tf.transpose(tf.tensordot(Rmatrix[i,...], tf.transpose(data[i,...]), [1,0]))		# Rotate the data. (num_points x 3)
		temp_rotated_data = tf.add(temp_rotated_data,translation[i,...])						# Add the translation (num_points x 3)
		transformed_data.append(temp_rotated_data)# Append data (batch_size x num_points x 3)
	transformed_data = tf.stack(transformed_data,0)					# Reshape data and remove first point cloud. (batch_size x num_point x 3)
	return transformed_data
...
def display_three_clouds(data1,data2,data3,title):
	...
	for row in data3:
		...
		Y.append(row[1])
		Z.append(row[2])
	l3 = ax.scatter(X,Y,Z,c=[0,0,1,0.5],marker='*')
	# ax4.scatter(X,Y,Z,c=[0,0,1,0.5],marker='*')
	# Add details to Plot.
...
def log_test_results(LOG_DIR, filename, log):
	...
	idxs_25_5, idxs_5_5, idxs_10_1, idxs_20_2 = log['idxs_25_5'],log['idxs_5_5'], log['idxs_10_1'], log['idxs_20_2']
	...
	if len(idxs_25_5) != 0:
		accuray_25_5 = len(idxs_25_5)/(num_batches*1.0)
		mean_25_5_rot_err = np.sum(np.array(Rot_Err)[idxs_25_5])/len(idxs_25_5)
		var_25_5_rot_err = np.var(np.array(Rot_Err)[idxs_25_5])
		mean_25_5_trans_err = np.sum(np.array(Trans_Err)[idxs_25_5])/len(idxs_25_5)
		var_25_5_trans_err = np.var(np.array(Trans_Err)[idxs_25_5])
		mean_25_5_itr = np.sum(np.array(ITR)[idxs_25_5])/len(idxs_25_5)
		var_25_5_itr = np.var(np.array(ITR)[idxs_25_5])
		mean_25_5_time = np.sum(np.array(TIME)[idxs_25_5])/len(idxs_25_5)
		var_25_5_time = np.var(np.array(TIME)[idxs_25_5])
	else:
		accuray_25_5, mean_25_5_rot_err, var_25_5_rot_err, mean_25_5_trans_err, var_25_5_trans_err, mean_25_5_itr, var_25_5_itr, mean_25_5_time, var_25_5_time = 0, 0, 0, 0, 0, 0, 0, 0, 0
	...
	with open(os.path.join(LOG_DIR, filename+'.txt'),'w') as file:
		...
		file.write("Mean Translation Err: {}\n".format(np.mean(Trans_Err)))
		file.write("Var Translation Err: {}\n".format(np.var(Trans_Err)))
		file.write("Mean Rotation Err: {}\n".format(np.mean(Rot_Err)))
		file.write("Var Rotation Err: {}\n".format(np.var(Rot_Err)))
		file.write("\n")

		file.write("###### 2.5 Degree & 0.05 Units ######\n")
		file.write("Accuray: {}%\n".format(accuray_25_5*100))
		file.write("Mean rotational error: {}\n".format(mean_25_5_rot_err))
		file.write("Mean translation error: {}\n".format(mean_25_5_trans_err))
		file.write("Mean time: {}\n".format(mean_25_5_time))
		file.write("Var time: {}\n".format(var_25_5_time))
		file.write("Var translation error: {}\n".format(var_25_5_trans_err))
		file.write("Var rotational error: {}\n".format(var_25_5_rot_err))
		file.write("Mean Iterations: {}\n".format(mean_25_5_itr))
	...
	plt.hist(Rot_Err,np.arange(0,185,1),density=True,cumulative=True)
	plt.xlim(0,180)
	plt.savefig(os.path.join(LOG_DIR,'rot_err_hist.jpeg'),dpi=500,quality=100)
	plt.figure()
	plt.hist(Trans_Err,np.arange(0,1.01,0.01),density=True,cumulative=True)
	plt.xlim(0,0.5)
	plt.savefig(os.path.join(LOG_DIR,'trans_err_hist.jpeg'),dpi=500,quality=100)
	plt.figure()
	plt.hist(Rot_Err,np.arange(0,185,0.5),density=True,cumulative=True)
	plt.xlim(0,20)
	plt.savefig(os.path.join(LOG_DIR,'rot_err_hist_low.jpeg'),dpi=500,quality=100)

def split_template_source(template_data,batch_euler_poses,NUM_POINT,centroid_subtraction_switch,ADD_NOISE,S_RAND_POINTS,SPARSE=1):
	if np.random.random_sample() < S_RAND_POINTS:
		...
		template_data = helper.select_random_points(template_data, (2 * NUM_POINT))
	...
	source_data = helper.apply_transformation(source_data,
									  batch_euler_poses)  # Apply the poses on the templates to get source data.
...
def add_occlusions(source_data,occlusions):
	num_points = source_data.shape[1]
	batch_size = source_data.shape[0]
	rand_ind = np.random.randint(0,num_points,batch_size)
	cur_len = (occlusions) * num_points
	source_data_occ = []
	for ii in range(batch_size):
		s = source_data[ii]
		rand_point = s[rand_ind[ii]]
		B_dist = np.linalg.norm(s - rand_point, 2, -1)
		indexes = np.argsort(B_dist)
		indexes = indexes[int(cur_len):]
		np.random.shuffle(indexes)
		s = s[indexes, ...]
		while s.shape[0] < num_points:
			s = np.concatenate([s, s], 0)
		s = s[:num_points, ...]
		source_data_occ.append(s)
	source_data_occ = np.stack(source_data_occ,0)
	return source_data_occ
